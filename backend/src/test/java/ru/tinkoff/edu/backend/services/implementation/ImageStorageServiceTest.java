...
class ImageStorageServiceTest {
  ...
  void deleteFromFileStorageLocation_withValidFile_deletesFile() throws Exception {
    ...
    Files.createFile(
        rootLocation.resolve(FileStorageLocation.USER_PROFILE_PHOTO.getPath()).resolve(fileName));
    ...
    imageStorageService.deleteFromFileStorageLocation(
        FileStorageLocation.USER_PROFILE_PHOTO, fileName);
  ...
  void save_emptyFile_thenReturnEmptyString() {
    ...
    String result =
        imageStorageService.save(FileStorageLocation.USER_PROFILE_PHOTO, emptyFile, fileName);
  ...
  void save_withUnsupportedFileExtension_throwsException() {
    ...
    MockMultipartFile unsupportedFile =
        new MockMultipartFile("image", fileName, "text/plain", new byte[123]);
    String messageExceptionExpected = "The file extension is not supported! File:" + fileName;

    String messageExceptionActual =
        assertThrows(
                FileExtensionNotSupportedException.class,
                () ->
                    imageStorageService.save(
                        FileStorageLocation.USER_CERTIFICATES, unsupportedFile, fileName))
            .getMessage();
  ...
  void save_withValidImage_returnsImageUrl() {
    ...
    MockMultipartFile validImage =
        new MockMultipartFile("image", fileName + ".jpg", "image/jpeg", "image".getBytes());
    String result =
        imageStorageService.save(FileStorageLocation.USER_PROFILE_PHOTO, validImage, fileName);

    assertTrue(result.contains(FileStorageLocation.USER_PROFILE_PHOTO.getApi()));
    assertTrue(
        Files.exists(
