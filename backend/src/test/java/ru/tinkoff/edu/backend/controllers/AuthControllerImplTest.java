...
class AuthControllerImplTest {
  ...
  User user_1 = User.builder().id(1L).email("123@example.com").password("123").build();
  ...
  void post_loginUserWithEntityNotFoundException_thenStatus400() throws Exception {
    when(userService.readByUserLoginDTO(userLoginDTO_1))
        .thenThrow(new EntityNotFoundException("User not exist!"));
  ...
  void post_registrationUser_withException_thenStatus201() throws Exception {
    when(userService.create(userRegDTO_1))
