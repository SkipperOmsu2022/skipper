...
public class MentorRepositoryImpl implements MentorRepository {
  ...
  public Page<User> getPageableMentors(FilterSortPaginationMentorListDTO dto) {
    ...
    query.setParameter(
        ...
        Arrays.stream(dto.getMentorSpecializations()).collect(Collectors.toSet()));
  ...
  private Page<User> getPageableUser(TypedQuery<User> query, int offset, int limit) {
    ...
    return new PageImpl<>(resultList, PageRequest.of(offset, limit), totalResults);
  }
}
