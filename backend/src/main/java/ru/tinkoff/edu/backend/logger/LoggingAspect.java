...
public class LoggingAspect {
    /**
     * Срез/запрос точек присоединения. Некая область с несколькими joinPoint.
     * Может быть одна или несколько точек/срезов в одном Pointcut.
     * Правила можно объединять через  &&, ||,!
     */
     ...
     * -Around — можно сделать пред., пост., обработку перед вызовом метода, а также вообще обойти вызов метода.<br>
     * <b>На один Pointcut можно «повесить» несколько Advice разного типа.</b>
     * @param joinPoint точка наблюдения, точка присоединения к коду, где планируется введение функциональности.
     */
    @Before("pointcut()")
