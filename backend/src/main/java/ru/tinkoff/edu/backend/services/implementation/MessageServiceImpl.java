...
/**
 * Transactional нужен, чтобы извлекать весь контекст в прокси, иначе вызывается исключение
...
public class MessageServiceImpl implements MessageService {
   ...
   */
  protected Conversation addMessageInConversation(
      Long userIdTo, Long userIdFrom, MessageDTO messageDTO) {
  ...
  @Override
  public List<ConversationDTO> getListConversations(
      Long userId, PaginationListConversationDTO dto) {
    return conversationRepository
        .getConversationsByUserId(
            PageRequest.of(dto.getOffsetConversations(), dto.getLimitConversations()), userId)
        .stream()
        .map(
            conversation ->
                conversationToConversationDTO(
                    ...
                    getListMessagesForConversation(
                        conversation.getId(), PageRequest.of(0, dto.getLimitMessages()))))
        .collect(Collectors.toList());
  }

  public List<MessageDTO> getListMessagesForConversation(
      Long userId1, Long userId2, PaginationListMessageDTO dto) {
    return getListMessagesForConversation(
        getConversation(
                userRepository.getReferenceById(userId1), userRepository.getReferenceById(userId2))
            .getId(),
        PageRequest.of(dto.getOffset(), dto.getLimit()));
  }

  /**
   * Возвращает Conversation по id пользователей, которые участвуют в диалоге. Иначе возвращает
   * новый объект.
   *
  ...
  protected Conversation getConversation(User user1, User user2) {
    return conversationRepository
        ...
        .orElseGet(
            () -> Conversation.builder().users(new HashSet<>(Arrays.asList(user1, user2))).build());
  }

  /** Возвращает постранично сообщения с конца в хронологическом порядке. */
  protected List<MessageDTO> getListMessagesForConversation(
      Long conversationId, Pageable pageable) {
    return messageToMessageDTOs(
            conversationRepository.getMessagesForConversation(pageable, conversationId))
        .stream()
